...
1. What role does the MCP server play in the MCP agent tool integration?
   ...
   * [x] Hosts tool definitions and makes them available for discovery by the client.

2. How does an MCP client retrieve available tools from the MCP server?
   * [x] By calling session.list_tools() to get the current tool catalog.
...
3. Why should MCP tools be wrapped in async functions on the client-side?
   ...
   * [x] To enable asynchronous invocation so the agent can call tools without blocking.
